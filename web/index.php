...
$app->get('/dashboard/history',function() use($app) {
    ...
    $sql = "select distinct(m.id), u1.username, u2.username, mt.name, mt.datetime from matches m join users u1 on u1.id = m.user_1 join users u2 on u2.id = m.user_2 join meet_types mt on mt.id = m.meet_types_id where user_1 = $user_id OR user_2 = $user_id";
    $stuff = array();
